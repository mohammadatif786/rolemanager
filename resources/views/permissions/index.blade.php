...
<div x-data="{ 
    ...
    showEditModal: false,
    editingPermission: { id: null, name: '', guard_name: 'web' },
            ...
            <div class="flex items-center gap-x-5">
                <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-600 shadow-xl shadow-indigo-200">
                    <svg class="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.744c0 5.273 3.69 9.689 8.598 10.711a11.99 11.99 0 008.598-10.711c0-1.339-.22-2.628-.626-3.832A12.012 12.012 0 0112 2.714z" />
                    </svg>
                </div>
                <div>
                    <h1 class="text-3xl font-black tracking-tight text-slate-900">System Permissions</h1>
                    <p class="mt-1 text-sm font-medium text-slate-500">Define granular access rules and security constraints.</p>
                </div>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/50">
                        <th class="px-8 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400">Permission Identity</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="group hover:bg-slate-50/80 transition-all duration-200">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-x-4">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-sm font-bold text-slate-900">{{ $permission->name }}</span>
                                    <span class="text-[11px] font-medium text-slate-400 tracking-wide">ID: #{{ $permission->id }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                        ...
                        <td class="px-8 py-6">
                            <div class="flex flex-col">
                                <span class="text-sm font-medium text-slate-600">{{ $permission->created_at->format('M d, Y') }}</span>
                                <span class="text-[11px] text-slate-400">{{ $permission->created_at->format('h:i A') }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <div class="flex justify-end items-center gap-x-2">
                                <button @click="editingPermission = { id: {{ $permission->id }}, name: '{{ $permission->name }}', guard_name: '{{ $permission->guard_name }}' }; showEditModal = true" class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white text-slate-400 shadow-sm ring-1 ring-inset ring-slate-200 hover:text-indigo-600 hover:ring-indigo-200 hover:bg-indigo-50 transition-all">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                </button>
                                <button @click="deletePermission({{ $permission->id }}, '{{ $permission->name }}')" class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white text-slate-400 shadow-sm ring-1 ring-inset ring-slate-200 hover:text-rose-600 hover:ring-rose-200 hover:bg-rose-50 transition-all">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                    </svg>
                                </button>
                                <form id="delete-permission-form-{{ $permission->id }}" action="{{ route('permissions.destroy', $permission->id) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
                    ...
                    <tr>
                        <td colspan="4" class="px-8 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-sm font-bold text-slate-900">No permissions found</h3>
                                <p class="text-xs text-slate-500 mt-1">Get started by defining your first system permission.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div x-show="showCreateModal"
                ...
                <div class="flex items-center gap-x-5 mb-8">
                    ...
                    <div>
                        <h3 class="text-2xl font-black text-slate-900">Define New Permission</h3>
                        <p class="text-sm font-medium text-slate-500">Configure granular access rule and context.</p>
                    </div>
                </div>
                ...
                <form action="{{ route('permissions.store') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <label class="block text-sm font-bold text-slate-900 mb-2">Permission Label</label>
                        <input type="text" name="name" required placeholder="e.g. view_reports" class="block w-full rounded-2xl border-slate-200 py-3.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                    </div>
                    <div>
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div x-show="showEditModal"
                ...
                <div class="flex items-center gap-x-5 mb-8">
                    ...
                    <div>
                        <h3 class="text-2xl font-black text-slate-900">Modify Permission</h3>
                        <p class="text-sm font-medium text-slate-500">Update identity and guard configuration.</p>
                    </div>
                </div>

                <form :action="'{{ route('permissions.update', ['id' => '_ID_']) }}'.replace('_ID_', editingPermission.id)" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-sm font-bold text-slate-900 mb-2">Permission Label</label>
                        <input type="text" name="name" x-model="editingPermission.name" required class="block w-full rounded-2xl border-slate-200 py-3.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-900 mb-2">Guard Context</label>
                        <select name="guard_name" x-model="editingPermission.guard_name" class="block w-full rounded-2xl border-slate-200 py-3.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                            <option value="web">Web Application</option>
                            <option value="api">API Endpoint</option>
                        </select>
                    </div>
